...
async function main() {
  ...
  for (const { name, owner } of nameWithOwnerList) {
    const result = await urql
      .query<GetIssueAndCommentsQuery, GetIssueAndCommentsQueryVariables>(
        GetIssueAndComments,
        { name, owner }
      )
      .toPromise();
    ...
    result.data?.repository?.issues.nodes?.forEach((issue) => {
      issue?.comments.nodes?.forEach((comment) => {
        ...
        infoShareActivityScore = infoShareActivityScore.plus(
          new Decimal(eachCommentBodyLength).dividedBy(
            new Decimal(eachCommentElapsedDate || 1) // 0で割るのを防ぐ
          )
        );
      });
    });
